...
def test_nanogpt_checkpoint_roundtrip(tmp_path) -> None:
    torch.manual_seed(1234)
    model = NanoGPTForCausalLM(NanoGPTConfig.tiny(vocab_size=32))
    input_ids = torch.randint(0, model.config.vocab_size, (1, 6))
    expected = model(input_ids=input_ids).logits

    save_checkpoint(model, tmp_path, step=12, extra={"name": "nanogpt-roundtrip"})
    loaded, metadata = load_checkpoint(tmp_path)
    actual = loaded(input_ids=input_ids).logits

    assert isinstance(loaded, NanoGPTForCausalLM)
    assert metadata["step"] == 12
    assert metadata["extra"] == {"name": "nanogpt-roundtrip"}
    assert torch.allclose(expected, actual)
