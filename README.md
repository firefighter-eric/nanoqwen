...
运行 GPT vs Qwen-like 预训练架构对比 smoke。这个 smoke 走轻量本地 text 路径，
用于快速检查入口；完整 suite 会固定 tokenizer、parquet 数据流、context、
batch、optimizer、seed 和 step/time budget；Qwen 配置按本地
...
head_dim=128`；GPT 约 30,462,976 参数，Qwen-like 使用
...
parquet 文档流、per-doc BOS、best-fit packing 和 `40*524288` token BPB
测评。先准备 autoresearch cache：
